...
test("garp ceilings prevent more than the specified number of an item", ()=> {
    //Arrange
    const options = [1,2,3];
    const ceilings = {
        maxes: [1, null, null],
    ...
    //Act
    const perms = garp(options, 3, 1, ceilings);
    //Assert
    let counter = new Array(4).fill(0);
    perms.map((perm) => {
        const oneCount = perm.reduce( (total, num) => (num == 1) ? total+1 : total, 0);
        counter[oneCount]++;
        //console.log(perm);
        expect(oneCount).toBeLessThanOrEqual(1);
    });
    console.log(counter);
});
